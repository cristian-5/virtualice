...
void vm::run(arr<u8> code) {
	...
	while (true) {
		switch (* i) {
			...
			case  op::_jump_r: i += (i8)getB(i + 1); continue;
			case  op::_jump_a: i = & code.data[stack.pop().n]; continue;
			case  op::_jump_z: POP_A  if (a.i ==   0) { i += (i8)getB(i + 1); continue; } else SKIP_NEXT(1); continue;
			case  op::_jump_o: POP_A  if (a.i ==   1) { i += (i8)getB(i + 1); continue; } else SKIP_NEXT(1); continue;
			case op::_jump_nz: POP_A  if (a.i !=   0) { i += (i8)getB(i + 1); continue; } else SKIP_NEXT(1); continue;
			case  op::_jump_e: POP_BA if (a.n == b.n) { i += (i8)getB(i + 1); continue; } else SKIP_NEXT(1); continue;
			case op::_jump_ne: POP_BA if (a.n != b.n) { i += (i8)getB(i + 1); continue; } else SKIP_NEXT(1); continue;
			case  op::_jump_l<typ::n>: POP_BA if (a.n <  b.n) { i += (i8)getB(i + 1); continue; } else SKIP_NEXT(1); continue;
			case  op::_jump_l<typ::i>: POP_BA if (a.i <  b.i) { i += (i8)getB(i + 1); continue; } else SKIP_NEXT(1); continue;
			case  op::_jump_l<typ::r>: POP_BA if (a.r <  b.r) { i += (i8)getB(i + 1); continue; } else SKIP_NEXT(1); continue;
			case op::_jump_le<typ::n>: POP_BA if (a.n <= b.n) { i += (i8)getB(i + 1); continue; } else SKIP_NEXT(1); continue;
			case op::_jump_le<typ::i>: POP_BA if (a.i <= b.i) { i += (i8)getB(i + 1); continue; } else SKIP_NEXT(1); continue;
			case op::_jump_le<typ::r>: POP_BA if (a.r <= b.r) { i += (i8)getB(i + 1); continue; } else SKIP_NEXT(1); continue;
			case op::_jump_ge<typ::n>: POP_BA if (a.n >= b.n) { i += (i8)getB(i + 1); continue; } else SKIP_NEXT(1); continue;
			case op::_jump_ge<typ::i>: POP_BA if (a.i >= b.i) { i += (i8)getB(i + 1); continue; } else SKIP_NEXT(1); continue;
			case op::_jump_ge<typ::r>: POP_BA if (a.r >= b.r) { i += (i8)getB(i + 1); continue; } else SKIP_NEXT(1); continue;
			case  op::_jump_g<typ::n>: POP_BA if (a.n >  b.n) { i += (i8)getB(i + 1); continue; } else SKIP_NEXT(1); continue;
			case  op::_jump_g<typ::i>: POP_BA if (a.i >  b.i) { i += (i8)getB(i + 1); continue; } else SKIP_NEXT(1); continue;
			case  op::_jump_g<typ::r>: POP_BA if (a.r >  b.r) { i += (i8)getB(i + 1); continue; } else SKIP_NEXT(1); continue;
