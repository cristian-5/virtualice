...
[[gnu::always_inline]]
u8  vm::loadB(void * p) { return (* ((u8  *)p)) & B_MASK; }
...
void vm::run(arr<u8> code) {
	...
	printf("%08X\n", loadD(i));
	if (code.count < 4 || (loadD(i) & 0xFFFFFF00) != 0x69636500) return;
	...
	while (true) {
		switch (* i) {
			...
			case op::_rest: break;
			case op::_const<dta::b>: PUSH({ .n = loadB(++i) }); break;
			case op::_const<dta::w>: PUSH({ .n = loadW(++i) }); SKIP(2); continue;
			case op::_const<dta::d>: PUSH({ .n = loadD(++i) }); SKIP(4); continue;
			case op::_const<dta::q>: PUSH({ .n = loadQ(++i) }); SKIP(8); continue;
			case op::_const_0: PUSH({ .n = 0ull }); break;
			case op::_const_1: PUSH({ .n = 1ull }); break;
			case op::_swap: POP_AB PUSH_AB break;
			case op::_clone_n: for (u64 j = loadB(++i) + 1; j > 0; j--) PUSH(stack.top()); break;
			case  op::_drop_n: stack.decreaseBy(loadB(++i)); break;
			case   op::_clone: PUSH(stack.top()); break;
			case    op::_drop: stack.decrease();  break;
			case    op::_fail: f = stack.pop().n; break;
			case    op::_flag: PUSH({ .n = f });  break;
			...
			case op::_memory_l<dta::b>: PUSH({ .n = loadB(memory.data + loadD(++i)) }); SKIP(4); continue;
			case op::_memory_l<dta::w>: PUSH({ .n = loadW(memory.data + loadD(++i)) }); SKIP(4); continue;
			case op::_memory_l<dta::d>: PUSH({ .n = loadD(memory.data + loadD(++i)) }); SKIP(4); continue;
			case op::_memory_l<dta::q>: PUSH({ .n = loadQ(memory.data + loadD(++i)) }); SKIP(4); continue;
			case op::_memory_s<dta::b>: POP_A storeB(memory.data + loadD(++i), a.n); SKIP(4); continue;
			case op::_memory_s<dta::w>: POP_A storeW(memory.data + loadD(++i), a.n); SKIP(4); continue;
			case op::_memory_s<dta::d>: POP_A storeD(memory.data + loadD(++i), a.n); SKIP(4); continue;
			case op::_memory_s<dta::q>: POP_A storeQ(memory.data + loadD(++i), a.n); SKIP(4); continue;
			case     op::_rotate: POP_BA PUSH({ .n = rotate( a.n, b.i) }); break;
			case   op::_rotate_r: POP_A  PUSH({ .n = rotateR(a.n, loadB(++i)) }); break;
			case   op::_rotate_l: POP_A  PUSH({ .n = rotateL(a.n, loadB(++i)) }); break;
			case      op::_shift: POP_BA PUSH({ .n = b.i < 0 ? (a.n >> - b.i) : (a.n << b.i) }); break;
			case    op::_shift_r: POP_A  PUSH({ .n = a.n >> loadB(++i) }); break;
			case    op::_shift_l: POP_A  PUSH({ .n = a.n << loadB(++i) }); break;
			...
			case   op::_bit: POP_A PUSH({ .n = get_bit(a.n, loadB(++i)) });   break;
			case op::_bit_0: POP_A PUSH({ .n = reset_bit(a.n, loadB(++i)) }); break;
			case op::_bit_1: POP_A PUSH({ .n = set_bit(a.n, loadB(++i)) });   break;
			...
			case  op::_jump_r: i += (i8)loadB(i + 1); continue;
			case  op::_jump_a: i = & code.data[stack.pop().n]; continue;
			case  op::_jump_z: POP_A  if (a.i ==   0) { i += (i8)loadB(i + 1); continue; } else SKIP_NEXT(1); continue;
			case  op::_jump_o: POP_A  if (a.i ==   1) { i += (i8)loadB(i + 1); continue; } else SKIP_NEXT(1); continue;
			case op::_jump_nz: POP_A  if (a.i !=   0) { i += (i8)loadB(i + 1); continue; } else SKIP_NEXT(1); continue;
			case  op::_jump_e: POP_BA if (a.n == b.n) { i += (i8)loadB(i + 1); continue; } else SKIP_NEXT(1); continue;
			case op::_jump_ne: POP_BA if (a.n != b.n) { i += (i8)loadB(i + 1); continue; } else SKIP_NEXT(1); continue;
			case  op::_jump_l<typ::n>: POP_BA if (a.n <  b.n) { i += (i8)loadB(i + 1); continue; } else SKIP_NEXT(1); continue;
			case  op::_jump_l<typ::i>: POP_BA if (a.i <  b.i) { i += (i8)loadB(i + 1); continue; } else SKIP_NEXT(1); continue;
			case  op::_jump_l<typ::r>: POP_BA if (a.r <  b.r) { i += (i8)loadB(i + 1); continue; } else SKIP_NEXT(1); continue;
			case op::_jump_le<typ::n>: POP_BA if (a.n <= b.n) { i += (i8)loadB(i + 1); continue; } else SKIP_NEXT(1); continue;
			case op::_jump_le<typ::i>: POP_BA if (a.i <= b.i) { i += (i8)loadB(i + 1); continue; } else SKIP_NEXT(1); continue;
			case op::_jump_le<typ::r>: POP_BA if (a.r <= b.r) { i += (i8)loadB(i + 1); continue; } else SKIP_NEXT(1); continue;
			case op::_jump_ge<typ::n>: POP_BA if (a.n >= b.n) { i += (i8)loadB(i + 1); continue; } else SKIP_NEXT(1); continue;
			case op::_jump_ge<typ::i>: POP_BA if (a.i >= b.i) { i += (i8)loadB(i + 1); continue; } else SKIP_NEXT(1); continue;
			case op::_jump_ge<typ::r>: POP_BA if (a.r >= b.r) { i += (i8)loadB(i + 1); continue; } else SKIP_NEXT(1); continue;
			case  op::_jump_g<typ::n>: POP_BA if (a.n >  b.n) { i += (i8)loadB(i + 1); continue; } else SKIP_NEXT(1); continue;
			case  op::_jump_g<typ::i>: POP_BA if (a.i >  b.i) { i += (i8)loadB(i + 1); continue; } else SKIP_NEXT(1); continue;
			case  op::_jump_g<typ::r>: POP_BA if (a.r >  b.r) { i += (i8)loadB(i + 1); continue; } else SKIP_NEXT(1); continue;
			...
			case op::_global_r: PUSH(stack[loadB(++i)]);         break;
			case op::_global_w: stack[loadB(++i)] = stack.pop(); break;
			case op::_global_e_r: PUSH(stack[loadW(++i)]); SKIP(2);         continue;
			case op::_global_e_w: stack[loadW(++i)] = stack.pop(); SKIP(2); continue;
			case op::_local_r_: PUSH(stack[fp + 1 + loadB(++i)]);         break;
			case op::_local_w_: stack[fp + 1 + loadB(++i)] = stack.pop(); break;
			...
			case    op::_arity<>: stack[fp].n |= (u64)loadB(++i) << 56; break;
			...
			case op::_param_r_: PUSH(stack[fp - 1 - loadB(++i)]);         break;
			case op::_param_w_: stack[fp - 1 - loadB(++i)] = stack.pop(); break;
